...
setTimeout(numberDeleted,30000);
const remember = []
...
function numberDeleted() {
    ...
    // for (let test = 0; test < randomSeries.length; test++) {
    //     let numberone = prompt(`prova a indovinare il numero ${test+1}`)
    //     if (numberone == randomSeries[test] ) {
    //         remember.push( "risposta giusta" );
    //     }else {
    //         remember.push("risposta errata");
    //     }

    //     number.innerHTML += `<div class="cell">  ${remember[test]} </div>`;
    // }
